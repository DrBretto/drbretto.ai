...
const StockStrengthIndicator = () => {
  ...
  const calculateConfidence = (data1, data2) => {
    const sumChanges = (data) =>
      data
        .slice(1)
        .reduce((acc, value, index) => acc + (value - data[index]), 0);
  ...
  const getBackgroundColor = (confidenceLevel) => {
    const confidenceValue = parseFloat(
      confidenceLevel.split('%')[0].split(' ')[5]
    );
    ...
    return confidenceValue > 50
      ? `#${greenColor}${Math.floor(alpha * 255)
          .toString(16)
          .padStart(2, '0')}`
      : `#${blueColor}${Math.floor(alpha * 255)
          .toString(16)
          .padStart(2, '0')}`;
  };
