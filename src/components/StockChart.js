...
const calculateSMA = (data, dataIndex, windowSize) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (i < windowSize - 1) {
      ...
      const sum = data.slice(i - windowSize + 1, i + 1).reduce((acc, val) => acc + val[dataIndex], 0);
...
const fetchLatestPrediction = async () => {
  try {
    ...
    const normalizedData = predictionData.stock1_prediction.map((value, index) => ({
      date: `Prediction ${index + 1}`,
      JDST: ((value - minJDST) / (maxJDST - minJDST)) * 100, // Normalize JDST data
      NUGT: ((predictionData.stock2_prediction[index] - minNUGT) / (maxNUGT - minNUGT)) * 100, // Normalize NUGT data
    }));
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      ...
      >
        ...
        <Tooltip />
        <Legend />
        <Line type="monotone" dataKey="JDST" stroke="#8884d8" strokeWidth={1} strokeOpacity={0.25} dot={false} />
        <Line type="monotone" dataKey="NUGT" stroke="#82ca9d" strokeWidth={1} strokeOpacity={0.25} dot={false} />
        <Line type="monotone" dataKey="JDST_SMA" stroke="blue" strokeWidth={3} dot={false} />
        <Line type="monotone" dataKey="NUGT_SMA" stroke="green" strokeWidth={3} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
};
